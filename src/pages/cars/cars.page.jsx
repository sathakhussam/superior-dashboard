...
class CarsPage extends Component {
  ...
  async componentDidMount() {
    ...
    const allCars = await (
      await API.get("cars/", { headers: { Authorization: `Bearer ${token}` } })
    ).data.data;
    allCar = allCars.cars;
  ...
  handleValueChange = (e) => {
    ...
    this.setState({ [e.target.name]: e.target.value }, () => {
      // let myFilteredCars;
      // if ()
      let myFilteredCars = allCar.filter((val) =>
        val.name.toLowerCase().includes(this.state.searchName)
      );
      // console.log(myFilteredCars)
      myFilteredCars = myFilteredCars.filter((val) =>
        val.type.toLowerCase().includes(this.state.searchType)
      );
      // console.log(myFilteredCars)
      myFilteredCars = myFilteredCars.filter((val) =>
        val.brand.includes(this.state.searchBrand)
      );
      // console.log(myFilteredCars)
      this.setState({
      ...
      <div className="OrdersPage">
        ...
        <Card customClass="custom-card">
          <div className="top-div">
            ...
            <Link className="btn btn-primary" to="/cars/create/new">
              Add new
            </Link>
          </div>
          <table class="rtable">
